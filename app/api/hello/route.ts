...
import { merkleTool } from "@/utils/tools/merkel" 
...
export async function POST(request: Request) {
	try {
		...
		// })
		// console.debug(account.privateKey.toHexString())

		const signer = new LocalSigner(account, Network.MAINNET)
		const aptosAgent = new AgentRuntime(signer, aptos, {
			PANORA_API_KEY: process.env.PANORA_API_KEY,
		})
		const tools= (createAptosTools(aptosAgent) as (StructuredToolInterface | DynamicTool | RunnableToolLike)[]).push(merkleTool)
		...
		// Add strategy context if linkData exists
		const strategyContext = linkData 
			? `\n\nThe user has provided a YouTube video about their investment strategy. The strategy is in a structured json format:\n ${linkData}. Consider this context when providing advice or executing transactions.`
			: ''

		// Create React agent with combined prompt
		const agent = createReactAgent({
			llm,
			tools,
			checkpointSaver: memory,
			messageModifier: basePrompt + strategyContext,
		})
